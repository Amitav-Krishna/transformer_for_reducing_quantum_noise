"""
Train Deep Hierarchical MLP on 5-qubit dataset (float64).

Depth control experiment: 2x more layers than Transformer (8+8 vs 4+4).

Specs:
- Model: HierarchicalMLP5QubitDeep (~2.15M params, 8+8 layers)
- Data: 32x32 density matrices, float64
- Batch size: 4
- Epochs: 100
- Output: checkpoints_16/mlp_5qubit_deep/
"""
...
def main():
    ...
    if os.path.exists("/workspace"):
        checkpoint_dir = "/workspace/checkpoints_16/mlp_5qubit_deep"
        csv_dir = "/workspace/csvs_16"
    else:
        checkpoint_dir = "checkpoints_16/mlp_5qubit_deep"
        csv_dir = "csvs_16"
    ...
    # Create model
    model = HierarchicalMLP5QubitDeep(loss_fn=FrobeniusFidelityLoss())
