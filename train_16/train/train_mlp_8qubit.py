"""
Train Hierarchical MLP (matched) on 8-qubit dataset (float64, streaming).

Specs:
- Model: HierarchicalMLP8Qubit (~1.61M params, same as Transformer)
- Data: 256x256 density matrices, float64, streaming
- Batch size: 4
- Epochs: 100
- Output: checkpoints_16/mlp_8qubit/
"""
...
def main():
    ...
    if os.path.exists("/workspace"):
        checkpoint_dir = "/workspace/checkpoints_16/mlp_8qubit"
        csv_dir = "/workspace/csvs_16"
    else:
        checkpoint_dir = "checkpoints_16/mlp_8qubit"
        csv_dir = "csvs_16"
    ...
    # Create model
    model = HierarchicalMLP8Qubit(loss_fn=FrobeniusFidelityLoss())
