...
\label{sec:org84a22a8}

\subsection{Scalable Attention Mechanisms}
\label{sec:org3b709fa}
...
\section{Methods}
\label{sec:org3827231}

\subsection{Dataset Generation}
\label{sec:org601ff83}
...
\label{sec:orgf5a6b79}

\subsubsection{Shared Hyperparameters}
\label{sec:org4d95514}
...
\section{Results}
\label{sec:orgfa3acd9}

\subsection{5-Qubit Results}
\label{sec:orgf0f74d2}

Table \ref{tab:org39e65e4} presents our primary comparison between hierarchical Transformer and MLP architectures on 5-qubit density matrices, trained with Frobenius normalization.

\begin{table}[htbp]
\caption{\label{tab:org39e65e4}5-qubit hierarchical model performance with Frobenius normalization. Both models use identical patch tokenization (\(4\times 4\) patches, 64 tokens) and matched parameter counts.}
\centering
\begin{tabular}{llrl}
...
\begin{table}[htbp]
\caption{\label{tab:org4fe68cf}8-qubit scaling results. The task is exponentially harder (baseline 0.006), but the MLP still achieves significant relative improvement.}
\centering
\begin{tabular}{lrrl}
...
Figure \ref{fig:orgdfe1b6e} visualizes the validation loss progression for all models, highlighting the stark contrast between 5-qubit and 8-qubit learning dynamics.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/val_loss_curves.pdf}
\caption{\label{fig:orgdfe1b6e}Validation loss curves. 5-qubit models (solid lines) show steady improvement over 100 epochs, while 8-qubit models (dashed lines) plateau immediately with minimal learning.}
\end{figure}
...
\label{sec:org05077dd}

Earlier experiments used single precision (float32), but we observed numerical instability when computing Uhlmann fidelity due to small negative eigenvalues arising from floating-point errors during eigendecomposition. Switching to double precision (float64) throughout the pipeline resolved these issues. We recommend float64 for quantum state computations where eigendecomposition is required (see Appendix \ref{sec:orge7dc685}).
\subsection{Input Normalization}
\label{sec:org2395e6a}

We initially observed unstable training dynamics, with wider and deeper MLP variants performing worse than the baseline model (see Appendix \ref{sec:orgbb20a8f}). Investigation revealed a scale mismatch: noisy density matrices have Frobenius norm \textasciitilde{}0.07, while clean targets have norm \textasciitilde{}1.0. Without normalization, models must simultaneously learn structural denoising and a \textasciitilde{}\(15\times\) scale correction, leading to optimization difficulties.
...
\label{sec:org37d5b9f}

\subsubsection{Patch Size Ceiling}
\label{sec:orgd126dd6}
...
\section{Appendix}
\label{sec:org4a5a073}

\subsection{Model Parameter Counts}
\label{sec:orgb17a855}
\begin{table}[htbp]
\caption{\label{tab:org7fd554e}Detailed parameter breakdown.}
\centering
\begin{tabular}{lllll}
...
\label{sec:org43818d9}
To validate Frobenius fidelity as a training proxy, we analyzed its correlation with the ground-truth Uhlmann fidelity. Figure \ref{fig:orgc3a43e3} shows the relationship for 100 randomly sampled 5-qubit density matrices.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/frob_vs_uhlmann_correlation.pdf}
\caption{\label{fig:orgc3a43e3}Correlation between Frobenius fidelity (normalized) and Uhlmann fidelity (physical) on a random subset of the 5-qubit dataset. The strong correlation (\(r=0.91\)) justifies using Frobenius fidelity as a training proxy.}
\end{figure}
\subsection{Single vs Double Precision}
\label{sec:orge7dc685}
...
\begin{table}[htbp]
\caption{\label{tab:org0c0294e}Capacity ablation: wider MLP performs worse, not better.}
\centering
\begin{tabular}{llrl}
...
\label{sec:org75b2766}

We tested whether the MLP required more depth by doubling layers from 4+4 to 8+8.

\begin{table}[htbp]
\caption{\label{tab:org9f93923}Depth ablation: deeper MLP performs worse, not better.}
\centering
\begin{tabular}{lrlrl}
