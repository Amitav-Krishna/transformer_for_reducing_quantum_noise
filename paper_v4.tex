...
\section{Methods}
\label{sec:orga31354a}

\subsection{Dataset Generation}
\label{sec:orgfec7e61}
...
\label{sec:org16182c3}

\subsubsection{Shared Hyperparameters}
\label{sec:org3c17bb4}
...
\section{Results}
\label{sec:org351f3e4}

\subsection{Effect of Frobenius Normalization}
\label{sec:orgb3fd041}

Our central finding is that Frobenius normalization dramatically improves training stability and final model performance. Table \ref{tab:org805786e} compares validation loss with and without normalization on the 5-qubit dataset.

\begin{table}[htbp]
\caption{\label{tab:org805786e}Effect of Frobenius normalization on 5-qubit MLP training. Normalization reduces validation loss by 25\% and enables stable convergence. *Unstable: capacity scaling hurts performance; see Appendix \ref{sec:org2c0345f}.}
\centering
\begin{tabular}{lrl}
...
\begin{table}[htbp]
\caption{\label{tab:org0c936c4}Capacity ablations on unnormalized 5-qubit data. Larger models perform worse, indicating optimization pathology rather than underfitting.}
\centering
\begin{tabular}{llrl}
...
\label{sec:org555b8f5}

With Frobenius normalization, both architectures achieve substantial improvements over the noisy baseline:

\begin{table}[htbp]
\caption{\label{tab:orgae69f4b}5-qubit model performance with Frobenius normalization. Both models use identical patch tokenization (\(4\times 4\) patches, 64 tokens) and matched parameter counts.}
\centering
\begin{tabular}{llrl}
Model & Params & Uhlmann Fidelity & vs Baseline\\
\hline
...
\begin{table}[htbp]
\caption{\label{tab:orgc1f2ea9}8-qubit scaling results with Frobenius normalization. Despite the exponentially harder task (baseline 0.006), both models achieve more than 3 times improvement.}
\centering
\begin{tabular}{lrl}
...
\label{sec:org55684b4}

Figure \ref{fig:org4f214a2} visualizes the validation loss progression, illustrating the stable convergence enabled by normalization.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/val_loss_curves.pdf}
\caption{\label{fig:org4f214a2}Validation loss curves with Frobenius normalization. 5-qubit models (solid lines) show rapid initial improvement followed by gradual convergence. 8-qubit models (dashed lines) learn more slowly but maintain steady progress.}
\end{figure}
...
\section{Discussion}
\label{sec:orged85e12}

\subsection{Why Normalization Matters}
\label{sec:org1862407}
...
\label{sec:org4c9907a}

\subsubsection{Patch Size Ceiling}
\label{sec:org12ce0b2}
...
\section{Appendix}
\label{sec:orgf32ed3d}

\subsection{Model Parameter Counts}
\label{sec:org01012c5}
\begin{table}[htbp]
\caption{\label{tab:orge59a5ff}Detailed parameter breakdown.}
\centering
\begin{tabular}{lllll}
...
\label{sec:org54a542a}
To validate Frobenius fidelity as a training proxy, we analyzed its correlation with the ground-truth Uhlmann fidelity. Figure \ref{fig:orgb64cbd0} shows the relationship for 100 randomly sampled 5-qubit density matrices.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/frob_vs_uhlmann_correlation.pdf}
\caption{\label{fig:orgb64cbd0}Correlation between Frobenius fidelity (normalized) and Uhlmann fidelity (physical) on a random subset of the 5-qubit dataset. The strong correlation (\(r=0.91\)) justifies using Frobenius fidelity as a training proxy.}
\end{figure}
\subsection{Single vs Double Precision}
\label{sec:org6d2af84}
...
\label{sec:orgd75f7c8}

We scaled MLP hidden dimensions by 4 times, increasing parameters from 1.09M to 5.29M.

\begin{table}[htbp]
\caption{\label{tab:orge2da87c}Capacity ablation without normalization: wider MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{llrl}
...
\label{sec:orgb8b4022}

We doubled the number of layers from 4+4 to 8+8.

\begin{table}[htbp]
\caption{\label{tab:orgfcaeb39}Depth ablation without normalization: deeper MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{lrlrl}
