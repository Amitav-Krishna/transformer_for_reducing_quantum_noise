...
\label{sec:org2f013fd}

Quantum state denoising---reconstructing clean density matrices from noisy measurements---is essential for extracting useful information from near-term quantum devices. While prior work has explored convolutional and feedforward architectures for this task \autocites{Morgillo_2024}[][]{kendre2025machinelearningquantumnoise}, the effectiveness of global attention mechanisms for quantum states with non-local entanglement correlations remains underexplored.
\subsection{The Scaling Problem}
\label{sec:orgd9019f2}
...
\begin{enumerate}
\item \textbf{\textbf{5-qubit Success}}: The Hierarchical Transformer achieves 3.14 times improvement over the baseline, outperforming the matched MLP.
\item \textbf{\textbf{8-qubit Scalability}}: The 8-qubit MLP achieves 3.76 times improvement over the baseline, proving that useful signal can be extracted even with large \(32\times 32\) patches.
...
\label{sec:orgf2a2133}

\subsection{Scalable Attention Mechanisms}
\label{sec:orge32aef0}

Standard self-attention scales quadratically with sequence length, prompting the development of linear-complexity approximations such as Linformer \autocite{wang2020linformer}, Performer \autocite{choromanski2020rethinking}, and BigBird \autocite{zaheer2020bigbird}. These methods typically use low-rank projections or sparse attention patterns to reduce computational cost. However, for quantum state denoising, we require dense all-to-all correlation modeling, which patch-based hierarchies preserve more effectively than sparse approximations.
\subsection{Hierarchical Vision Transformers}
\label{sec:orgc99bb3f}

Our approach is inspired by Vision Transformers (ViT) \autocite{dosovitskiy2020image} and hierarchical variants like the Swin Transformer \autocite{liu2021swin}, which process images as sequences of patches. While ViT maintains a constant resolution, Swin builds a hierarchy by merging patches. We adopt the patch-tokenization strategy but maintain a fixed resolution (64 tokens) tailored to the specific density matrix dimensions (32x32 or 256x256), effectively adjusting the "patch size" rather than the number of layers to handle scaling.
\subsection{Quantum Machine Learning for Error Correction}
\label{sec:org9d6646e}
...
\section{Methods}
\label{sec:orgf5f2bda}

\subsection{Dataset Generation}
\label{sec:org7a49c45}
...
\label{sec:org9de4d76}

\subsubsection{Shared Hyperparameters}
\label{sec:orgeaf746c}
...
\section{Results}
\label{sec:orgdb71a43}

\subsection{5-Qubit Results}
\label{sec:org98ce5ce}



\begin{table}[htbp]
\caption{\label{tab:org612b1de}5-qubit hierarchical model performance with Frobenius normalization. Both models use identical patch tokenization (\(4\times 4\) patches, 64 tokens) and matched parameter counts.}
\centering
\begin{tabular}{llrl}
...
\end{tabular}
\end{table}
Table \ref{tab:org612b1de} presents our primary comparison between hierarchical Transformer and MLP architectures on 5-qubit density matrices, trained with Frobenius normalization.

Both architectures achieve substantial improvements over the noisy baseline, with the MLP achieving 3.09 times improvement and the Transformer achieving 3.14 times improvement. Notably, with proper input normalization, the gap between MLP and Transformer narrows considerably---the Transformer's advantage is \((sim 1.7\%)\) relative (0.525 vs 0.516). For the unnormalized experiments and the full comparison, see Appendix \ref{sec:orgba52606}.
...
\begin{table}[htbp]
\caption{\label{tab:org611ea35}8-qubit scaling results. The task is exponentially harder (baseline 0.006), but the MLP still achieves significant relative improvement.}
\centering
\begin{tabular}{lrrl}
...
Figure \ref{fig:orgd2b1fa0} visualizes the validation loss progression for all models, highlighting the stark contrast between 5-qubit and 8-qubit learning dynamics.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/val_loss_curves.pdf}
\caption{\label{fig:orgd2b1fa0}Validation loss curves. 5-qubit models (solid lines) show steady improvement over 100 epochs, while 8-qubit models (dashed lines) plateau immediately with minimal learning.}
\end{figure}
...
\label{sec:orge7288e8}

\subsubsection{Patch Size Ceiling}
\label{sec:org15c4e93}
...
\textbf{5-qubit success.} On 5-qubit systems (\(32\times 32\) matrices), the hierarchical Transformer achieves 3.14 times improvement over the noisy baseline, maintaining a clear advantage over a parameter-matched MLP (3.09 times). Ablations with wider and deeper MLPs performed significantly worse, confirming that the attention mechanism provides a specific inductive bias that simple capacity scaling cannot replicate.
...
\section{Appendix}
\label{sec:orgbfbd563}

\subsection{Model Parameter Counts}
\label{sec:orge63cabd}
\begin{table}[htbp]
\caption{\label{tab:orgcfedb71}Detailed parameter breakdown.}
\centering
\begin{tabular}{lllll}
...
\label{sec:org922071a}
To validate Frobenius fidelity as a training proxy, we analyzed its correlation with the ground-truth Uhlmann fidelity. Figure \ref{fig:org7e5e618} shows the relationship for 100 randomly sampled 5-qubit density matrices.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/frob_vs_uhlmann_correlation.pdf}
\caption{\label{fig:org7e5e618}Correlation between Frobenius fidelity (normalized) and Uhlmann fidelity (physical) on a random subset of the 5-qubit dataset. The strong correlation (\(r=0.91\)) justifies using Frobenius fidelity as a training proxy.}
\end{figure}
\subsection{Single vs Double Precision}
\label{sec:orgdcfb8b7}
...
\begin{table}[htbp]
\caption{\label{tab:org32cb491}Capacity ablation: wider MLP performs worse, not better.}
\centering
\begin{tabular}{llrl}
...
\label{sec:org11391b7}

We tested whether the MLP required more depth by doubling layers from 4+4 to 8+8.

\begin{table}[htbp]
\caption{\label{tab:org079b208}Depth ablation: deeper MLP performs worse, not better.}
\centering
\begin{tabular}{lrlrl}
