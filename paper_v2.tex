...
\section{Methods}
\label{sec:org7105f37}
\subsection{Dataset}
\label{sec:org0a01ce5}
...
\label{sec:orga8dd808}

\subsubsection{Shared Hyperparameters}
\label{sec:orgc5df5ee}
...
\label{sec:orge1fb544}
\subsubsection{It was an RTX A4000}
\label{sec:orgc11deb7}
\newpage
\section{Results}
\label{sec:orgc09b510}
...
\end{tabular}
\end{center}\footnotetext[1]{\label{org15771c9}Reconstruction losses are only comparable within the same loss family (Frobenius vs. Frobenius, Physics vs. Physics). Cross-family comparisons have no physical meaning.}
...
\caption{\label{fig:org8735442}Baseline Uhlmann fidelity between noisy inputs and clean target states across noise types and levels. Lower values indicate more severe corruption.}
\end{figure}
...
\caption{\label{fig:org1411ff1}Reconstruction fidelity comparison. The baseline (gray) shows the fidelity of noisy inputs to clean targets. The Frobenius-trained transformer achieves near-perfect reconstruction (+0.84 improvement), while the physics-informed CNN performs worse than doing nothing.}
\end{figure}
...
\section{References}
\label{sec:org1ff4008}
\printbibliography
\subsection{{\bfseries\sffamily TODO} Add more citations}
\label{sec:orga2f749c}
\section{Appendix}
\label{sec:org507861a}
\end{document}
