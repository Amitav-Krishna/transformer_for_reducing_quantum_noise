...
\section{Methods}
\label{sec:orgf1edf99}
\subsection{Dataset}
\label{sec:orgf624427}
...
\label{sec:org41e9002}

The physics term enforces the basic structural axioms of a valid density matrix during reconstruction. Given a predicted state (\(\hat{\rho}\)), the loss penalizes violations of Hermiticity, trace normalization, and positive semidefiniteness, which are the three core constraints defining legitimate quantum states\autocite{Torlai2018}. Hermiticity is enforced by measuring the deviation between (\(\hat{\rho}\)) and its conjugate transpose, trace preservation is encouraged by penalizing deviations of (\(\mathrm{Tr}(\hat{\rho})\)) from unity, and positivity is promoted by penalizing negative eigenvalues of (\(\hat{\rho}\)). Together, these terms guide the model toward producing physically consistent outputs, irrespective of the noise model or architecture.
...
\label{sec:org37d9342}

\subsubsection{Shared Hyperparameters}
\label{sec:org2acc3ec}
...
\label{sec:org4d9687c}
\subsubsection{It was an RTX A4000}
\label{sec:orgfa7a52a}
\newpage
\section{Results}
\label{sec:org1038b36}
...
\end{tabular}
\end{center}\footnotetext[1]{\label{org6f4c159}Reconstruction losses are only comparable within the same loss family (Frobenius vs. Frobenius, Physics vs. Physics). Cross-family comparisons have no physical meaning.}
...
\caption{\label{fig:orgdd80475}Baseline Uhlmann fidelity between noisy inputs and clean target states across noise types and levels. Lower values indicate more severe corruption.}
\end{figure}
...
\caption{\label{fig:org42936f7}Reconstruction fidelity comparison. The baseline (gray) shows the fidelity of noisy inputs to clean targets. The Frobenius-trained transformer achieves near-perfect reconstruction (+0.84 improvement), while the physics-informed CNN performs worse than doing nothing.}
\end{figure}
...
\section{References}
\label{sec:org0a69e9d}
\printbibliography
\subsection{{\bfseries\sffamily TODO} Add more citations}
\label{sec:orgd2281eb}
\section{Appendix}
\label{sec:org871773e}
\end{document}
