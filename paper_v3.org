...
* Conclusion
We have demonstrated that architectural inductive bias significantly impacts neural network performance for quantum state denoising. With comparable parameter counts (~120k), the Transformer autoencoder achieves [TBD]× higher Uhlmann fidelity than the noisy baseline, compared to [TBD]× for the MLP—a [TBD]× advantage attributable to the self-attention mechanism's ability to model correlations between arbitrary matrix elements.
...
* Footnotes
* Appendix
** Failed Experiment: Cholesky-Constrained Output Layer
