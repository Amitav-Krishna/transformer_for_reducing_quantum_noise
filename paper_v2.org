...
\includegraphics[width=\linewidth]{figures/cnn_frob_train_loss.png}
\caption{CNN Frobenius}
\end{subfigure}
\begin{subfigure}{0.45\linewidth}
\includegraphics[width=\linewidth]{figures/cnn_physics_train_loss.png}
\caption{CNN Physics}
\end{subfigure}

\begin{subfigure}{0.45\linewidth}
\includegraphics[width=\linewidth]{figures/transformer_frob_train_loss.png}
\caption{Transformer Frobenius}
\end{subfigure}
\begin{subfigure}{0.45\linewidth}
\includegraphics[width=\linewidth]{figures/transformer_physics_train_loss.png}
...
\centering
\begin{subfigure}{0.45\linewidth}
\includegraphics[width=\linewidth]{figures/cnn_frob_test_fidelity.png}
\caption{CNN Frobenius}
\end{subfigure}
\begin{subfigure}{0.45\linewidth}
\includegraphics[width=\linewidth]{figures/cnn_physics_test_fidelity.png}
\caption{CNN Physics}
\end{subfigure}

\begin{subfigure}{0.45\linewidth}
\includegraphics[width=\linewidth]{figures/transformer_frob_test_fidelity.png}
\caption{Transformer Frobenius}
\end{subfigure}
\begin{subfigure}{0.45\linewidth}
\includegraphics[width=\linewidth]{figures/transformer_physics_test_fidelity.png}
\caption{Transformer Physics}
\end{subfigure}
\end{figure}
#+END_EXPORT
...
* Conclusion
This work compared four machine learning strategies for density-matrix denoising and showed that architectural bias dominates loss-design when reconstructing quantum states. The Frobenius-trained transformer achieved the highest fidelity, followed by the physics-informed transformer, while both CNN variants underperformed due to their inability to capture global quantum correlations. The physics-informed loss improved the CNN by constraining it away from pathological outputs but reduced transformer performance by over-penalizing structural deviations that occur during accurate reconstruction. These findings clarify how architectural capacity and physical regularization interact, and they identify attention-based models as the most promising class for scalable quantum error mitigation. Improving classical denoisers directly improves the effective reliability of NISQ-era experiments, which remains essential while fully fault-tolerant quantum hardware is still out of reach.
* References
#+print_bibliography:
