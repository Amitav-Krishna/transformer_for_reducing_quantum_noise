...
* Abstract
Denoising entangled quantum states requires capturing global structure and long-range correlations in density matrices, yet it remains unclear whether architectural inductive biases or raw model capacity drive performance. We compare convolutional and transformer autoencoders on synthetic density-matrix denoising across 20 noise settings, using Uhlmann fidelity to measure reconstruction quality. From a baseline of 0.11 fidelity for noisy inputs, a transformer with only 119k parameters achieves 0.95 fidelity, while a CNN with 749k parameters reaches just 0.30. The transformer closes 94% of the gap to perfect reconstruction compared to the CNN's 21%. Counterintuitively, scaling the transformer to 751k parameters to match CNN capacity /degrades/ performance to 0.87 fidelity, suggesting that the transformer's advantage is consistent with architectural properties rather than parameter count. Adding soft physics-based regularization (Hermiticity, positive semi-definiteness, unit trace) harms both architectures. These results suggest that architectural inductive bias, not capacity, is the primary factor in denoising efficacy for this task, though the precise mechanism requires further investigation.
* Introduction
...
$$
where ($\langle \hat{\rho}, \rho \rangle_F = \mathrm{Tr}(\hat{\rho}^\dagger \rho)$) is the Frobenius inner product and ($\lVert \cdot \rVert_F$) is the Frobenius norm. This formulation is analogous to cosine similarity for complex matrices: it measures the alignment between predicted and target states while normalizing for magnitude differences. The normalized inner product is bounded above by 1 via the Cauchy-Schwarz inequality; for valid density matrices (positive semi-definite), the Frobenius inner product is non-negative, restricting the normalized similarity to [0, 1] and hence the loss to [0, 1]. During training, before outputs are fully constrained, the loss can exceed 1.

Unlike the squared Frobenius distance ($\lVert \hat{\rho} - \rho \rVert_F^2$), the normalized similarity is scale-invariant and focuses on structural alignment rather than absolute elementwise error. This property is desirable for density-matrix reconstruction, where preserving the relative structure of coherences matters more than matching raw magnitudes. The Uhlmann fidelity remains the correct operational measure of quantum state similarity, but its reliance on matrix square roots makes it approximately 30× more expensive to evaluate during training for 32×32 matrices. While the correlation between the Frobenius similarity and Uhlmann fidelity is strongly nonlinear (Fig. 1), we observe a very strong rank correlation between the two metrics (Spearman's \(\rho\) = 0.95, p \(\ll\) 10^(-10)), indicating that improvements under the Frobenius objective consistently correspond to improvements in Uhlmann fidelity.
...
$$
  ...
  where ($\lambda_i(\hat{\rho})$) are the eigenvalues of ($\hat{\rho}$). In all experiments, we use \(\lambda_{\mathrm{herm}} = \lambda_{\mathrm{trace}} = \lambda_{\mathrm{psd}} = 1.0\), weighting all three constraint violations equally. The total training loss combines this term with the normalized fidelity loss
...
Beyond architecture and loss function, several additional factors differ between the CNN and transformer configurations and represent potential confounds. Specifically, the CNN uses ReLU activations while the transformer uses GELU; the CNN uses no dropout while the transformer uses dropout of 0.1; and batch sizes differ (32 for CNN, 8 for transformer) due to memory constraints. These differences were not systematically ablated and could contribute to observed performance gaps independently of the core architectural distinction between convolution and attention.
** Hyperparameters
...
| Dropout              | 0                                                     | 0.1                                         |
| Bottleneck           | —                                                     | Linear 96 $\rightarrow$ 48 $\rightarrow$ 96 |
...
#+LATEX:\newpage
* Results
Transformer autoencoders dramatically outperform CNNs for density-matrix denoising, and this advantage is consistent with architectural inductive bias rather than model capacity. We evaluated CNN and transformer architectures across two loss functions (Frobenius and physics-informed) and two transformer capacities (119k and 751k parameters), providing evidence that global attention mechanisms, not parameter count, are associated with reconstruction quality.

*Note on Uhlmann fidelity computation:* Because the Uhlmann fidelity formula requires Hermitian inputs, we symmetrize all outputs via \((\hat{\rho} + \hat{\rho}^\dagger)/2\) before evaluation. This enforces Hermiticity but does not correct trace deviations or negative eigenvalues. Eigenvalue clamping occurs only inside the matrix square root computation for numerical stability, not as post-processing on the output. This means the soft Hermiticity penalty in the physics-informed loss may be partially redundant at evaluation time, though it still affects the optimization trajectory during training.

The smaller transformer (119k parameters) achieves 0.95 Uhlmann fidelity, outperforming both the CNN (749k parameters, 0.30 fidelity) and a capacity-matched transformer (751k parameters, 0.87 fidelity). Physics-informed regularization degrades both architectures, with the physics-informed CNN performing worse than the noisy baseline. These patterns appear consistently across training curves, test metrics, and noise-resolved evaluations.
*** Charts :noexport:
...
| Architecture       | Parameters | Loss Function | Uhlmann Fidelity | Improvement |
|--------------------+------------+---------------+------------------+-------------|
| Baseline           | —          | (Noisy input) |     0.11 ± 0.15 |           — |
| CNN                |       749k | Frobenius     |     0.30 ± 0.28 |       +0.19 |
| CNN                |       749k | Physics       |     0.06 ± 0.05 |       −0.05 |
| Transformer (119k) |       119k | Frobenius     |    *0.95 ± 0.12* |       +0.84 |
| Transformer (119k) |       119k | Physics       |     0.33 ± 0.33 |       +0.22 |
| Transformer (751k) |       751k | Frobenius     |     0.87 ± 0.30 |       +0.76 |
| Transformer (751k) |       751k | Physics       |     0.26 ± 0.28 |       +0.15 |
...
The Frobenius-trained transformer (119k) achieves 0.95 mean fidelity, an improvement of +0.84 over the noisy baseline. This recovers nearly all of the structure destroyed by the noise channels. The Frobenius-trained CNN provides a modest +0.19 improvement despite having six times more parameters. Most strikingly, the physics-informed CNN achieves only 0.06 fidelity, which is *worse than the noisy input itself* (−0.05). This suggests that the physics constraints, rather than guiding the model toward valid reconstructions, cause it to collapse toward generic low-fidelity states that happen to satisfy Hermiticity, trace, and positivity constraints.
...
The capacity-matched transformer (751k) achieves 0.87 fidelity with Frobenius loss. This represents strong performance, but it falls notably below the smaller transformer's 0.95. This counterintuitive result is examined in detail in the Capacity Ablation subsection.
...
A natural objection to the transformer's dominance is that it might simply benefit from a different parameter regime or optimization landscape rather than from attention per se. To test this, we trained a capacity-matched transformer with 751k parameters, within 0.3% of the CNN's 749k, using embedding dimension 96 and feed-forward width 128. If parameter count were the mechanism behind the transformer's success, the larger model should outperform the smaller one.

The results refute this hypothesis. The transformer (119k) achieves 0.95 Uhlmann fidelity with Frobenius loss, while the transformer (751k) achieves only 0.87. This represents a degradation of 0.08 despite a sixfold increase in parameters. The same pattern holds for physics-informed training: the smaller transformer reaches 0.33 fidelity versus 0.26 for the larger variant. This inversion cannot be explained by underfitting, since the 751k model has strictly greater capacity. Instead, it suggests that the additional parameters introduce optimization difficulties or overfitting on this task, while the smaller model's constrained capacity forces it to learn more generalizable representations.

These findings are consistent with the hypothesis that the transformer's advantage over CNNs is architectural rather than parametric. The attention mechanism's ability to model correlations across the entire density matrix in a single layer may explain this advantage, though we note that sufficiently deep CNNs can also achieve large receptive fields in principle. Scaling the transformer to match CNN capacity does not close the gap; it widens it. For this density-matrix denoising task, architectural inductive bias appears more important than raw model size, though ablation studies isolating specific architectural components would be needed to confirm the causal mechanism.
...
* Conclusion
This work compared CNN and transformer autoencoders across loss functions and model capacities for density-matrix denoising. The Frobenius-trained transformer (119k parameters) achieved 0.95 Uhlmann fidelity on the test set. To provide operational context, F=0.95 corresponds to a trace distance of at most \(\sqrt{1-0.95^2} \approx 0.31\) via the Fuchs-van de Graaf inequality, meaning the reconstructed and target states remain distinguishable with probability up to ~66% under optimal measurement. A capacity-matched transformer (751k parameters) achieved only 0.87 and the CNN (749k parameters) reached just 0.30. This counterintuitive result, in which the smaller model outperforms the larger, suggests that architectural properties rather than parameter count are associated with the transformer's advantage. The precise mechanism remains to be isolated through targeted ablation studies.

Contrary to initial expectations, physics-informed regularization degraded performance for both architectures when evaluated on Uhlmann fidelity. The physics-informed CNN achieved only 0.06 fidelity compared to 0.30 for its Frobenius-trained counterpart, and the physics-informed transformer dropped from 0.95 to 0.33. While the physics constraints encourage structural validity (Hermiticity, unit trace, positive semidefiniteness), they appear to do so at the expense of accurate state reconstruction, possibly by pushing models toward generic outputs that satisfy physical axioms but fail to capture the specific structure of the target state.

These findings have practical implications for quantum error mitigation. First, transformer architectures outperform CNNs on this density-matrix denoising task, possibly due to their ability to model correlations across the full matrix in each layer. Second, soft physics constraints applied during training can be counterproductive when the goal is high-fidelity reconstruction; if physical validity is required, post-hoc projection onto the valid state manifold may be preferable to regularization. Third, evaluation on training-aligned metrics can be misleading, and neutral measures like Uhlmann fidelity should be reported alongside proxy losses.

** Limitations

Several limitations constrain the interpretation of these results. First, our causal claims about attention mechanisms are correlational: we have not performed ablation studies removing attention or comparing with other global-context architectures (e.g., MLPs on flattened inputs). Second, the noise channels studied are synthetic and may not reflect the noise characteristics of specific quantum hardware. Third, we study only 5-qubit systems (32×32 density matrices); scaling behavior to larger systems is unknown. Finally, the claim that CNNs "cannot" capture nonlocal correlations is imprecise; sufficiently deep CNNs can achieve arbitrarily large receptive fields, though they may do so less efficiently than attention mechanisms.
