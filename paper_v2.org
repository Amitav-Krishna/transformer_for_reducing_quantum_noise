...
** DONE Modify framing to an inversion of known noise channels
* Introduction
...
The dominant paradigm for neural density-matrix denoising uses convolutional autoencoders. Kendre et al. introduced a CNN-based approach that treats the density matrix as a two-channel image and applies standard encoder-decoder convolutions to map noisy states to clean reconstructions [cite:@kendre2025machinelearningquantumnoise]. This architecture exploits local spatial correlations but cannot directly model the nonlocal quantum correlations present in entangled states. Morgillo et al. extended this line of work with feedforward networks for states corrupted by unknown noise channels, demonstrating model-agnostic denoising but without addressing architectural limitations [cite:@Morgillo_2024].

An alternative thread enforces physical validity through hard constraints. Hradil et al. proposed positivity-constrained feedforward networks that project outputs onto the space of valid quantum states, guaranteeing Hermiticity and positive semidefiniteness by construction [cite:@PhysRevA.106.012409]. While this ensures physical consistency, it does not address whether the network itself learns representations suited to quantum structure.
...
#+CAPTION: Transformer autoencoder for density-matrix denoising. (a) The architecture flattens the $32 \times 32$ complex density matrix into 1024 tokens, each embedded from 2D (real/imaginary) to 32D. The encoder applies self-attention across all tokens, the bottleneck compresses the representation, and the decoder reconstructs the clean state via cross-attention to the encoder memory. (b) Unlike CNNs, every matrix element attends globally to every other element, enabling the model to capture nonlocal quantum correlations.
