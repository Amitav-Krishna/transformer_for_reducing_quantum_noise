...
\section{Methods}
\label{sec:orgfe5e1d2}
\subsection{Dataset}
\label{sec:orgbc10ff0}
...
\label{sec:orga4a8f91}
\subsubsection{Shared Hyperparameters}
\label{sec:org4179376}
...
\section{Results}
\label{sec:org92c141b}
\subsection{Training Dynamics}
\label{sec:org7267dd4}
...
\label{sec:orgc0b0e1e}

Table \ref{tab:org227be42} summarizes the reconstruction fidelity of both architectures compared to the baseline (noisy input without denoising).

\begin{table}[htbp]
\caption{\label{tab:org227be42}Uhlmann fidelity on the held-out test set. Higher is better. Baseline represents the fidelity between noisy and clean states without any denoising.}
\centering
\begin{tabular}{lll}
...
\begin{table}[htbp]
\caption{\label{tab:org29acb68}Capacity scaling analysis. The wide MLP has 44× more parameters than the Transformer but achieves essentially identical performance to the small MLP, demonstrating that capacity alone cannot explain the Transformer's advantage.}
\centering
\begin{tabular}{llrrr}
...
\subsection{Depth Control: Ruling Out Insufficient Layers}
\label{sec:org1d3bdb4}

A natural follow-up question: is the shallow MLP's underperformance due to insufficient depth rather than architectural unsuitability? To definitively test this, we trained a deep residual MLP with identical parameter count (\textasciitilde{}121.6k) but 8 nonlinear transformations (vs 2 for shallow), per-layer residual connections, long skip connections, and GELU activation (smoother than ReLU for regression).

\begin{table}[htbp]
\caption{\label{tab:org063cce0}Depth control experiment. A deep residual MLP with 8 nonlinear transformations and equivalent parameter count to the shallow MLP achieves worse validation loss, ruling out insufficient depth as the limiting factor.}
\centering
\begin{tabular}{llrr}
...
\subsection{Representation Control: Pauli Basis Removes Spatial Structure}
\label{sec:org4370bb4}

A final control tests whether the Transformer exploits \textbf{spatial layout} of the density matrix or \textbf{algebraic entanglement structure}. The density matrix representation encodes quantum states as 32×32 grids where physically related elements (e.g., coherences between entangled basis states) occupy specific grid positions. An architecture might succeed by learning these spatial patterns rather than understanding entanglement itself.

To disentangle these factors, we transform the density matrix to the \textbf{Pauli basis}: any n-qubit density matrix can be written as
...
\begin{table}[htbp]
\caption{\label{tab:org971a6bd}Pauli basis representation control. The Transformer improves when spatial structure is removed, while the MLP collapses to worse-than-baseline performance.}
\centering
\begin{tabular}{llll}
...
The results are striking. The Transformer \textbf{improves} when spatial structure is removed (0.28 → 0.33 Uhlmann fidelity), suggesting it was partially distracted by grid-locality patterns that don't reflect true entanglement structure. In the Pauli basis, the Transformer can directly model algebraic relationships between coefficients without spatial interference.
...
\end{figure}
\subsection{CNN Baseline: Spatial Locality Bias Fails}
\label{sec:org57b85ff}

To rule out that any architecture with \textasciitilde{}120k parameters could succeed, we trained a CNN autoencoder as an additional baseline. CNNs impose strong spatial locality bias through their convolutional kernels, which is appropriate for images but potentially mismatched to quantum states where correlations span arbitrary matrix elements.

The CNN uses a standard encoder-decoder architecture with channel progression 2→19→38→76→76→38→19→2, MaxPooling for downsampling, and nearest-neighbor upsampling with transposed convolutions. Like the MLP and Transformer, it uses unconstrained outputs with post-hoc projection to valid density matrices. Total parameters: \textasciitilde{}118k, closely matching the other architectures.

\begin{table}[htbp]
\caption{\label{tab:orgda35c73}CNN baseline with post-hoc projection. Despite comparable parameter count, the CNN performs worse than the noisy baseline.}
\centering
\begin{tabular}{llll}
...
The CNN performs \textbf{worse} than the baseline (0.68× improvement, i.e., degradation), confirming that spatial locality bias is actively harmful for quantum state denoising. Convolutional kernels assume that nearby elements are more correlated than distant ones—but in entangled states, correlations exist between elements separated across the matrix (e.g., \(\rho_{00,00}\) and \(\rho_{31,31}\) for GHZ states). The CNN's inductive bias directly contradicts the structure of the problem.

A natural objection is that sufficiently deep CNNs can achieve global receptive fields. Our CNN's effective receptive field spans the entire 32×32 matrix through its encoder-decoder architecture with pooling. However, the failure is not about receptive field \emph{size} but about the \emph{nature} of the learned representations. Even with global receptive field, CNNs construct features hierarchically from local patterns—they learn that nearby elements form meaningful groups (edges, textures, regions). For quantum states, this hierarchical composition is incorrect: the correlation between \(\rho_{00,31}\) and \(\rho_{31,00}\) (conjugate off-diagonal elements ensuring Hermiticity) is not mediated by intermediate spatial locations. The 2-channel (real/imaginary) image representation treats the matrix as a spatial grid, but entanglement correlations follow algebraic structure (trace constraints, eigenvalue relationships) that does not respect grid locality at any scale.
...
\begin{table}[htbp]
\caption{\label{tab:orga4982b1}Cholesky-constrained models perform worse than the noisy baseline. The Transformer collapses to the maximally mixed state (fidelity \(\approx 1/32\)).}
\centering
\begin{tabular}{llll}
Model & Parameters & Uhlmann Fidelity & vs Baseline\\
\hline
...
\newpage
\section{Conclusion}
\label{sec:orgbc3d7a1}
We have demonstrated that architectural inductive bias—not model capacity, depth, or representation format—determines neural network performance for quantum state denoising. With comparable parameter counts (\textasciitilde{}120k), the Transformer autoencoder achieves 2.3× higher Uhlmann fidelity than the noisy baseline, compared to 1.4× for the MLP and 0.68× for the CNN (which actively degrades states). Four control experiments reinforce this conclusion:

\begin{enumerate}
\item \textbf{CNN baseline}: A CNN with identical parameter count performs \textbf{worse} than doing nothing (0.68× baseline fidelity), confirming that spatial locality bias is fundamentally wrong for quantum states where entanglement induces non-local correlations.

\item \textbf{Capacity control}: A wide MLP with 5.2 million parameters (44× the Transformer's count) achieves no improvement over the small MLP (0.587 validation loss), demonstrating that parameter count alone cannot explain the Transformer's advantage.

\item \textbf{Depth control}: A deep residual MLP with 8 nonlinear transformations (vs 2 for shallow), per-layer residuals, long skip connections, and GELU activation actually performs \textbf{worse} (0.765 vs 0.587 validation loss) than the shallow baseline, ruling out insufficient depth as the limiting factor.

\item \textbf{Representation control}: When inputs are transformed to the Pauli basis—removing spatial structure while preserving algebraic entanglement relationships—the Transformer \textbf{improves} to 2.7× while the MLP collapses to 0.45× (worse than no denoising). This demonstrates that the Transformer learns algebraic entanglement structure, while the MLP requires spatial scaffolding to function at all.
\end{enumerate}
...
\subsection{Limitations: Noise Model and Real-World Applicability}
\label{sec:org7d59338}

Our dataset uses synthetic Markovian noise channels (depolarizing, amplitude damping, phase damping, bit-flip) applied independently to each qubit. Real quantum hardware exhibits additional complexity that our current study does not address:

\textbf{Non-Markovian dynamics.} Memory effects in quantum noise can cause error correlations across time, where the noise affecting a qubit depends on its recent history. Our sequential noise composition (mixed channel) does not induce such temporal correlations—each channel acts instantaneously without memory.

\textbf{Crosstalk and spatially correlated errors.} In physical devices, operations on one qubit can inadvertently affect neighboring qubits through electromagnetic coupling or shared control lines. This induces spatial correlations in the noise that differ from our independent-per-qubit model.

Interestingly, the Transformer's attention mechanism may be well-suited to handle such correlated errors. Non-Markovian dynamics and crosstalk both introduce correlations between matrix elements that would not exist under independent Markovian noise. Since attention can learn arbitrary pairwise correlations without assuming independence, the same architectural advantage that enables denoising of entanglement-induced correlations might extend to noise-induced correlations. However, this remains speculative—empirical validation on datasets with realistic correlated noise is needed. Future work should evaluate performance on noise models calibrated to specific hardware backends (e.g., IBM, Google, or trapped-ion devices) where crosstalk and non-Markovian effects are characterized.
\subsection{Scalability Analysis and Proposed Mitigations}
\label{sec:orgdc1e771}
...
\begin{itemize}
\item Density matrix dimension: \(2^n \times 2^n\)
\item Number of tokens: \((2^n)^2 = 4^n\)
\item Naive attention complexity: \(O((4^n)^2) = O(16^n)\) per layer
\end{itemize}

This creates a hard scalability barrier:

\begin{table}[htbp]
\caption{\label{tab:org7e807eb}Computational complexity scaling with qubit count. Element-wise tokenization becomes prohibitive beyond \textasciitilde{}6 qubits.}
\centering
\begin{tabular}{rllll}
Qubits & Matrix Size & Tokens & Naive Attention & Feasibility\\
\hline
5 & 32×32 & 1,024 & 1M operations & ✓ (our work)\\
6 & 64×64 & 4,096 & 16M operations & \textasciitilde{} (expensive)\\
...
However, density matrices have strong \textbf{structure} that naive attention ignores. The Transformer's success on 5-qubit systems suggests that attention mechanisms can exploit this structure when it's available. Three concrete mitigations are tractable:
\subsubsection{Block-Sparse Attention (Quantum-Aware)}
\label{sec:org67e48b9}
...
\begin{enumerate}
\item \textbf{Local attention} within blocks (intra-qubit correlations)
\item \textbf{Entanglement-respecting patterns} between blocks (only attend to causally-connected qubits)
\end{enumerate}

This reduces complexity from \(O(16^n)\) to \(O(4^n \cdot k)\) where \(k\) is block size. For \(n=7\) qubits, this is a \textasciitilde{}100× reduction.
\subsubsection{Low-Rank Attention Approximations}
\label{sec:org665c4e3}
...
\label{sec:orge172d02}

Instead of treating each 32×32 element as a token, we can:
\begin{itemize}
\item Group correlated elements into meta-tokens (e.g., 8×8 blocks → 16 tokens)
\item Use local convolutional layers within blocks, global attention across blocks
\item This reduces sequence length from 1024 to \textasciitilde{}64 for 5 qubits, enabling 10+ qubit scaling with modified architecture
\end{itemize}
\subsubsection{Empirical Outlook}
\label{sec:org6322609}
...
\section{References}
\label{sec:org9dc4704}
\bibliographystyle{unsrtnat}
\bibliography{references}
\section{Appendix}
\label{sec:org8c5b58d}
\subsection{Cholesky and CNN Experiment Architecture Details}
\label{sec:orgb592d71}

The Cholesky-constrained models use architectures matched to the post-hoc projection baselines:
\begin{itemize}
\item \textbf{MLP Cholesky} (\textasciitilde{}88k parameters): Same hidden structure as post-hoc MLP (2048→28→28), but outputs 1024 Cholesky parameters instead of 2048 reconstruction values. The Cholesky layer converts these to a valid density matrix via \(\rho = LL^\dagger / \text{Tr}(LL^\dagger)\).
\item \textbf{Transformer Cholesky} (\textasciitilde{}153k parameters): Same encoder-decoder structure as post-hoc Transformer (embed\textsubscript{dim}=32, 4 layers, 4 heads), but uses global attention pooling over decoder outputs followed by projection to 1024 Cholesky parameters. The additional global pooling layer accounts for the parameter increase.
\end{itemize}

The CNN baseline uses:
\begin{itemize}
\item \textbf{CNN} (\textasciitilde{}118k parameters): Encoder-decoder with channel progression 2→19→38→76 (encoder) and 76→38→19→2 (decoder). Uses 3×3 convolutions, MaxPool2d for downsampling, and nearest-neighbor upsampling with transposed convolutions. Post-hoc projection applied to outputs.
\end{itemize}
\end{document}
