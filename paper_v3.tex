...
\section{Methods}
\label{sec:org129bcd2}
\subsection{Dataset}
\label{sec:org8cf0906}
...
\label{sec:orgf3f62bc}
\subsubsection{Shared Hyperparameters}
\label{sec:org984b381}
...
\section{Results}
\label{sec:org900dc4b}
\subsection{Training Dynamics}
\label{sec:org304090a}
...
\label{sec:org6014ec6}

Table \ref{tab:org5a511e3} summarizes the reconstruction fidelity of both architectures compared to the baseline (noisy input without denoising).

\begin{table}[htbp]
\caption{\label{tab:org5a511e3}Uhlmann fidelity on the held-out test set. Higher is better. Baseline represents the fidelity between noisy and clean states without any denoising.}
\centering
\begin{tabular}{lll}
...
\begin{table}[htbp]
\caption{\label{tab:org6345f87}Capacity scaling analysis. The wide MLP has 44× more parameters than the Transformer but achieves essentially identical performance to the small MLP, demonstrating that capacity alone cannot explain the Transformer's advantage.}
\centering
\begin{tabular}{llrrr}
...
\begin{table}[htbp]
\caption{\label{tab:org8afae8d}Depth control experiment. A deep residual MLP with 8 nonlinear transformations and equivalent parameter count to the shallow MLP achieves worse validation loss, ruling out insufficient depth as the limiting factor.}
\centering
\begin{tabular}{llrr}
...
\begin{table}[htbp]
\caption{\label{tab:org80f13fa}Pauli basis representation control. The Transformer improves when spatial structure is removed, while the MLP collapses to worse-than-baseline performance.}
\centering
\begin{tabular}{llll}
...
\begin{table}[htbp]
\caption{\label{tab:orgb792a8c}Cholesky-constrained models perform worse than the noisy baseline. The Transformer collapses to the maximally mixed state (fidelity \(\approx 1/32\)).}
\centering
\begin{tabular}{ll}
...
\newpage
\section{Conclusion}
\label{sec:org851e80c}
We have demonstrated that architectural inductive bias—not model capacity, depth, or representation format—determines neural network performance for quantum state denoising. With comparable parameter counts (\textasciitilde{}120k), the Transformer autoencoder achieves 2.3× higher Uhlmann fidelity than the noisy baseline, compared to 1.4× for the MLP—a 1.6× advantage attributable to the self-attention mechanism's ability to model correlations between arbitrary matrix elements. Three control experiments reinforce this conclusion:

\begin{enumerate}
\item \textbf{Capacity control} (Section 5.5): A wide MLP with 5.2 million parameters (44× the Transformer's count) achieves no improvement over the small MLP (0.587 validation loss), demonstrating that parameter count alone cannot explain the Transformer's advantage.

\item \textbf{Depth control} (Section 5.6): A deep residual MLP with 8 nonlinear transformations (vs 2 for shallow), per-layer residuals, long skip connections, and GELU activation actually performs \textbf{worse} (0.765 vs 0.587 validation loss) than the shallow baseline, ruling out insufficient depth as the limiting factor.

\item \textbf{Representation control} (Section 5.7): When inputs are transformed to the Pauli basis—removing spatial structure while preserving algebraic entanglement relationships—the Transformer \textbf{improves} to 2.7× while the MLP collapses to 0.45× (worse than no denoising). This demonstrates that the Transformer learns algebraic entanglement structure, while the MLP requires spatial scaffolding to function at all.
\end{enumerate}

These results collectively demonstrate that the feedforward architecture fundamentally lacks the inductive bias needed to exploit the non-local structure of entangled quantum states. The Transformer succeeds not through greater capacity, but through a qualitatively different mechanism—attention routing that maintains token-level dimensionality (1024 dimensions) rather than compressing to a narrow bottleneck.
...
\begin{table}[htbp]
\caption{\label{tab:orgb8435fa}Computational complexity scaling with qubit count. Element-wise tokenization becomes prohibitive beyond \textasciitilde{}6 qubits.}
\centering
\begin{tabular}{rllll}
...
\section{References}
\label{sec:org3d18ad7}
\bibliographystyle{unsrtnat}
\bibliography{references}
\section{Appendix}
\label{sec:orgf87c8e9}
\subsection{Cholesky Experiment Architecture Details}
\label{sec:org2102b87}
