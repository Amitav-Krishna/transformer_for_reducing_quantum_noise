...
\section{Methods}
\label{sec:orga79dee9}
\subsection{Dataset}
\label{sec:orgda302a5}
...
\label{sec:org0501b50}
\subsubsection{Shared Hyperparameters}
\label{sec:org1c7c21e}
...
\section{Results}
\label{sec:orgb027553}
\subsection{Training Dynamics}
\label{sec:orgc492e57}
...
\label{sec:org4ba7950}

Table \ref{tab:org336bb58} summarizes the reconstruction fidelity of both architectures compared to the baseline (noisy input without denoising).

\begin{table}[htbp]
\caption{\label{tab:org336bb58}Uhlmann fidelity on the held-out test set. Higher is better. Baseline represents the fidelity between noisy and clean states without any denoising.}
\centering
\begin{tabular}{lll}
...
\begin{table}[htbp]
\caption{\label{tab:orgb333543}Capacity scaling analysis. The wide MLP has 44× more parameters than the Transformer but achieves essentially identical performance to the small MLP, demonstrating that capacity alone cannot explain the Transformer's advantage.}
\centering
\begin{tabular}{llrrr}
...
\begin{table}[htbp]
\caption{\label{tab:org5a18740}Cholesky-constrained models perform worse than the noisy baseline. The Transformer collapses to the maximally mixed state (fidelity \(\approx 1/32\)).}
\centering
\begin{tabular}{ll}
