...
\section{Methods}
\label{sec:orgf9792ad}

\subsection{Dataset Generation}
\label{sec:org8b03c6b}
...
\label{sec:org42ea1bc}

\subsubsection{Shared Hyperparameters}
\label{sec:org062fa93}
...
\section{Results}
\label{sec:org31d0013}

\subsection{Effect of Frobenius Normalization}
\label{sec:orgb9c09fc}

Our central finding is that Frobenius normalization dramatically improves training stability and final model performance. Table \ref{tab:org3ea2b94} compares validation loss with and without normalization on the 5-qubit dataset.

\begin{table}[htbp]
\caption{\label{tab:org3ea2b94}Effect of Frobenius normalization on 5-qubit MLP training. Normalization reduces validation loss by 25\% and enables stable convergence. *Unstable: capacity scaling hurts performance; see Appendix \ref{sec:org3156b09}.}
\centering
\begin{tabular}{lrl}
...
\begin{table}[htbp]
\caption{\label{tab:orge874752}Capacity ablations on unnormalized 5-qubit data. Larger models perform worse, indicating optimization pathology rather than underfitting.}
\centering
\begin{tabular}{llrl}
...
\label{sec:orgcfd13d0}

With Frobenius normalization, both architectures achieve substantial improvements over the noisy baseline:

\begin{table}[htbp]
\caption{\label{tab:org6e6ecc3}5-qubit model performance with Frobenius normalization. Both models use identical patch tokenization (\(4\times 4\) patches, 64 tokens) and matched parameter counts.}
\centering
\begin{tabular}{llrl}
Model & Params & Uhlmann Fidelity & vs Baseline\\
\hline
...
\begin{table}[htbp]
\caption{\label{tab:orgf213711}8-qubit scaling results with Frobenius normalization. Despite the exponentially harder task (baseline 0.006), both models achieve more than 3 times improvement.}
\centering
\begin{tabular}{lrl}
...
\label{sec:orgf2c7b1b}

\subsubsection{Patch Size Ceiling}
\label{sec:org636f0a0}
...
\section{Appendix}
\label{sec:orgc39af13}

\subsection{Model Parameter Counts}
\label{sec:org318a54c}
\begin{table}[htbp]
\caption{\label{tab:org7776841}Detailed parameter breakdown.}
\centering
\begin{tabular}{lllll}
...
\label{sec:orgbdb331a}
To validate Frobenius fidelity as a training proxy, we analyzed its correlation with the ground-truth Uhlmann fidelity. Figure \ref{fig:org57bcc18} shows the relationship for 100 randomly sampled 5-qubit density matrices.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/frob_vs_uhlmann_correlation.pdf}
\caption{\label{fig:org57bcc18}Correlation between Frobenius fidelity (normalized) and Uhlmann fidelity (physical) on a random subset of the 5-qubit dataset. The strong correlation (\(r=0.91\)) justifies using Frobenius fidelity as a training proxy.}
\end{figure}
\subsection{Single vs Double Precision}
\label{sec:org7a827a5}
...
\label{sec:org78f432d}

We scaled MLP hidden dimensions by 4 times, increasing parameters from 1.09M to 5.29M.

\begin{table}[htbp]
\caption{\label{tab:orgd8650ad}Capacity ablation without normalization: wider MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{llrl}
...
\label{sec:org9bb49a6}

We doubled the number of layers from 4+4 to 8+8.

\begin{table}[htbp]
\caption{\label{tab:orgc1be02e}Depth ablation without normalization: deeper MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{lrlrl}
