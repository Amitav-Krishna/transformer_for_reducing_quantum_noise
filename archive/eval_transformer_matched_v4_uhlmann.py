"""
Evaluate transformer_matched_v4 models on Uhlmann fidelity.

Run after training completes:
    python eval_transformer_matched_v4_uhlmann.py

Outputs:
    - Prints mean Uhlmann fidelity for both variants
    - Saves to csvs_2/transformer_matched_v4_uhlmann_results.csv
"""
...
def main():
    ...
    models_to_eval = {
        "transformer_matched_v4_frob": FrobeniusFidelityLoss(),
        "transformer_matched_v4_physics": CompositePhysicsTotalLoss(),
    }
    ...
    for name, loss_fn in models_to_eval.items():
        ...
        model = TransformerAutoencoderMatchedV4(loss_fn=loss_fn)
    ...
    if results:
        ...
        df.to_csv("csvs_2/transformer_matched_v4_uhlmann_results.csv", index=False)
        print(f"\nResults saved to csvs_2/transformer_matched_v4_uhlmann_results.csv")
