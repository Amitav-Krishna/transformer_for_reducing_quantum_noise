"""
Train Element-wise Cholesky Transformer.

This addresses the reviewer critique that the original Cholesky Transformer
used global pooling, making it an unfair comparison with the post-hoc version.

The element-wise Cholesky Transformer maintains the same architecture as
the post-hoc Transformer, just with each token outputting 1 value (for
Cholesky parameters) instead of 2 (for direct reconstruction).

Checkpoints saved to: train_13/checkpoints_13/
Training logs saved to: train_13/csvs_13/

Usage:
    python train_13/cholesky/train.py
"""
...
def main():
    ...
    # Directories
    ckpt_dir = os.path.join(base_dir, "checkpoints_13", "transformer_cholesky_elementwise")
    csv_dir = os.path.join(base_dir, "csvs_13")
    ...
    # CSV logger
    csv_logger = CSVLogger(csv_dir=csv_dir, name="transformer_cholesky_elementwise")
    ...
    print(f"\n{'='*60}")
    print(f"Training Element-wise Cholesky Transformer")
    ...
    print("\n" + "="*60)
    print("Element-wise Cholesky Transformer training complete!")
    print(f"Checkpoint saved to: {ckpt_dir}")
    print(f"Training logs saved to: {csv_dir}")
    print("="*60)
