"""
Train Wide Hierarchical MLP on 5-qubit dataset.

Capacity control experiment: Tests whether hierarchical Transformer's
advantage comes from attention or just from having more parameters.

Comparison:
- Hierarchical Transformer: ~1.1M params
- Hierarchical MLP (matched): ~1.1M params
- Hierarchical MLP Wide: ~5.2M params (this script)
...
def main():
    ...
    if os.path.exists("/workspace/dataset_smaller"):
        ...
        checkpoint_dir = "/workspace/checkpoints_2/hierarchical_mlp_5qubit_wide"
        csv_dir = "/workspace/csvs_2"
    else:
        dataset_dir = "dataset_smaller"
        checkpoint_dir = "checkpoints_2/hierarchical_mlp_5qubit_wide"
        csv_dir = "csvs_2"
    ...
    # Create model
    model = HierarchicalMLP5QubitWide(loss_fn=FrobeniusFidelityLoss()).to(device)
    num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model parameters: {num_params:,}")

    # Optimizer (same as other models)
    ...
    # CSV loggers
    train_csv_path = os.path.join(csv_dir, "hierarchical_mlp_5qubit_wide_train.csv")
    val_csv_path = os.path.join(csv_dir, "hierarchical_mlp_5qubit_wide_val.csv")
    timing_csv_path = os.path.join(csv_dir, "hierarchical_mlp_5qubit_wide_timing.csv")
