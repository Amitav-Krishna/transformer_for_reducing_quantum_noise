"""
Evaluate Hierarchical Transformer (8-qubit) on Uhlmann fidelity.

Outputs:
- csvs_2/hierarchical_transformer_8qubit_test_uhlmann.csv (overall metrics)
- csvs_2/noise_cells/hierarchical_transformer_8qubit_noise_cells.csv (per-noise breakdown)
"""
...
from training_loop.dataset.load_chunks import load_chunks, load_chunks_with_metadata
...
@torch.no_grad()
def evaluate_uhlmann_on_chunks(model, chunks, device, batch_size=4):
    """
...
@torch.no_grad()
def evaluate_baseline_uhlmann(chunks, device, batch_size=4):
    """
...
def main():
    ...
    if os.path.exists("/workspace/dataset_8qubit"):
        ...
        checkpoint_path = (
            "/workspace/checkpoints_2/hierarchical_transformer_8qubit/best.pt"
        )
        csv_dir = "/workspace/csvs_2"
    else:
        dataset_dir = "dataset_8qubit"
        checkpoint_path = "checkpoints_2/hierarchical_transformer_8qubit/best.pt"
        csv_dir = "csvs_2"

    os.makedirs(csv_dir, exist_ok=True)
    os.makedirs(os.path.join(csv_dir, "noise_cells"), exist_ok=True)

    # Load dataset
    print(f"Loading 8-qubit dataset from {dataset_dir}...")
    ...
    results_path = os.path.join(
        csv_dir, "hierarchical_transformer_8qubit_test_uhlmann.csv"
    )
    ...
    # Summary
    print(f"\n{'=' * 50}")
    print("SUMMARY: 8-Qubit Hierarchical Transformer")
    print(f"{'=' * 50}")
