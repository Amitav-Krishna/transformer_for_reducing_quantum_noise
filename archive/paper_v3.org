...
1. *CNN baseline*: A CNN with identical parameter count performs *worse* than doing nothing (0.68× baseline fidelity), confirming that spatial locality bias is fundamentally wrong for quantum states where entanglement induces non-local correlations.

2. *Capacity control*: A wide MLP with 5.2 million parameters (44× the Transformer's count) achieves no improvement over the small MLP (0.587 validation loss), demonstrating that parameter count alone cannot explain the Transformer's advantage.
...
** Limitations: Noise Model and Real-World Applicability
...
*Non-Markovian dynamics.* Memory effects in quantum noise can cause error correlations across time, where the noise affecting a qubit depends on its recent history. Our sequential noise composition (mixed channel) does not induce such temporal correlations—each channel acts instantaneously without memory.

*Crosstalk and spatially correlated errors.* In physical devices, operations on one qubit can inadvertently affect neighboring qubits through electromagnetic coupling or shared control lines. This induces spatial correlations in the noise that differ from our independent-per-qubit model.
...
Instead of treating each matrix element as a token, we group elements into spatial patches:
- 4×4 patches reduce 32×32 → 64 tokens (16× reduction)
- 32×32 patches reduce 256×256 → 64 tokens (1024× reduction)
- Local convolutional embedding within patches, global attention across patches

We implemented and trained hierarchical transformers on both 5-qubit and 8-qubit systems to validate this approach empirically.

*** Empirical Validation: Hierarchical Transformer Results

We trained a hierarchical transformer on the 5-qubit dataset using 4×4 patch tokenization (64 tokens vs 1024 for element-wise). Table [[tab:hierarchical]] compares the approaches:

#+NAME: tab:hierarchical
#+CAPTION: Hierarchical vs element-wise tokenization on 5-qubit density matrices. Hierarchical tokenization achieves reasonable performance with 16× fewer tokens and 250× fewer attention operations, though the larger patch embedding layers increase total parameters.
| Model | Tokens | Attention Ops | Parameters | Val Loss | Time/Epoch |
|-------|--------|---------------|------------|----------|------------|
| Element-wise Transformer | 1,024 | ~1M | 119k | 0.457 | ~300s |
| Hierarchical Transformer | 64 | ~4k | 1.1M | 0.579 | ~60s |

The hierarchical approach trains 5× faster per epoch due to reduced attention complexity, despite having 9× more parameters (the additional parameters are in the patch embedding/unembedding layers, not the attention mechanism). The validation loss gap (0.457 vs 0.579) represents the cost of losing fine-grained element-level attention—patch-level attention cannot capture correlations between individual matrix elements within the same patch.

For 8-qubit systems (256×256 matrices), hierarchical tokenization is essential—element-wise tokenization would require 65,536 tokens with 4 billion attention operations per layer. Our 8-qubit hierarchical transformer uses 32×32 patches, reducing to 64 tokens while maintaining the same attention complexity as the 5-qubit hierarchical model. Training on the 8-qubit dataset (100k samples, streaming from disk to handle the 98GB dataset size) is ongoing.

The key insight is that hierarchical tokenization trades spatial resolution for scalability. For applications requiring maximum fidelity, element-wise tokenization remains superior on systems where it is computationally feasible (≤6 qubits). For larger systems, hierarchical approaches provide a practical path forward, with the attention mechanism still capturing inter-patch correlations that MLPs and CNNs cannot learn.
* References
#+print_bibliography:
