"""
Evaluate wide MLP per noise cell (noise_type × noise_level).
Outputs CSVs for heatmap generation.

Run:
    python train_v9/eval_per_noise_cell.py
"""
...
@torch.no_grad()
def evaluate_per_noise_cell(model, chunks_with_meta, device, batch_size):
...
def main():
    ...
    # Output directory
    out_dir = "csvs_9/noise_cells"
    os.makedirs(out_dir, exist_ok=True)

    # === Wide MLP (v9) ===
    print("\nEvaluating Wide MLP (v9)...")
    mlp = MLPWideAutoencoder(loss_fn=FrobeniusFidelityLoss()).to(device)
    mlp.load_state_dict(torch.load("checkpoints_9/mlp_wide/best.pt", map_location=device))
    mlp_results = evaluate_per_noise_cell(mlp, test_chunks, device, 64)
    mlp_df = aggregate_results(mlp_results)
    mlp_df.to_csv(f"{out_dir}/mlp_wide_noise_cells.csv", index=False)
    print(f"  Saved: {out_dir}/mlp_wide_noise_cells.csv")
