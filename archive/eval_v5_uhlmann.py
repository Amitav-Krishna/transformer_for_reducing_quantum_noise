"""
Evaluate v5 models on Uhlmann fidelity with post-hoc projection.

Since outputs are unconstrained, we project to valid density matrices
...
from models_5.mlp import MLPAutoencoder
from models_5.transformer import TransformerAutoencoder
...
def project_to_density_matrix(rho):
    ...
    1. Symmetrize (Hermitianize)
    2. Project to positive semi-definite
    3. Normalize trace to 1

    Args:
        rho: (N, N) complex matrix
    ...
    eigvals = torch.clamp(eigvals, min=0)  # Remove negative eigenvalues
...
def uhlmann_fidelity_batch(rho_pred, rho_true, project=True):
    ...
    Args:
        ...
        project: If True, project predictions to valid density matrices
...
def evaluate_uhlmann_on_chunks(model, chunks, arch, device, batch_size):
    ...
    for X, Y in chunks:
        ...
        for x, y in loader:
            ...
            pred = model(x)
            # Project predictions to valid density matrices before computing fidelity
...
def main():
    ...
    for name, config in EXPERIMENTS.items():
        arch = config["arch"]
        ckpt_path = f"checkpoints_5/{name}/best.pt"
    ...
    # Summary table
    print(f"\n{'='*50}")
    print("SUMMARY: Uhlmann Fidelity on Test Set (v5 Models)")
    print(f"{'='*50}")
