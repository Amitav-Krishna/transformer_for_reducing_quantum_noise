"""
Train MLP and Transformer v6 with matched parameters (~120k each).

v6 uses the proven v2 Transformer architecture (element-wise tokenization).

Checkpoints saved to: checkpoints_6/
Training logs saved to: csvs_6/

Usage:
    python train_v6/train.py
"""
...
def train_single_experiment(name, config, train_chunks, val_chunks, device):
    ...
    # Directories
    ckpt_dir = f"checkpoints_6/{name}"
    os.makedirs(ckpt_dir, exist_ok=True)

    # CSV logger
    csv_logger = CSVLogger(csv_dir="csvs_6", name=name)
...
def main():
    ...
    os.makedirs("csvs_6", exist_ok=True)
    ...
    print("Checkpoints saved to: checkpoints_6/")
    print("Training logs saved to: csvs_6/")
    print("="*60)
