"""
Generate 6-qubit dataset for scalability proof-of-concept.

Circuit structure (matching 5-qubit dataset pattern):
- Depth = 6-9 layers (random per circuit)
- Each layer: single-qubit gates on ALL 6 qubits + ~3 two-qubit gates
- Total: ~54-81 single-qubit gates + ~18-27 two-qubit gates per circuit

This creates significant entanglement and allows noise to accumulate,
similar to the 5-qubit dataset.

Output: 64×64 density matrices (4096 elements)
Target: 100,000 samples (~6.4 GB)
...
def generate_circuit_6qubit(num_qubits=6, depth=None, seed=None):
    """
    Generate a 6-qubit circuit with specified depth.

    Each depth layer contains:
    - Single-qubit gates on ALL qubits
    - ~3 two-qubit gates on random adjacent pairs

    This matches the 5-qubit dataset generation pattern.

    Args:
        num_qubits: Number of qubits (default 6)
...
def generate_6qubit_dataset(
    output_dir="dataset_6qubit",
    ...
    """
    Generate 6-qubit dataset and stream to disk.
    ...
    print(f"Generating {total_samples} 6-qubit samples...")
    print(f"Matrix size: 64×64 (4096 elements)")
        ...
        for noise_type in noise_types:
            for noise_level in noise_levels:
                for i in range(num_samples_per_cell):
                    circuit, qubits = generate_circuit_6qubit(num_qubits=6)
                    # qubits are already LineQubit.range(6) in order
                    ...
                    # Verify shape
                    assert x_sample.shape == (64, 64, 2), f"Bad X shape: {x_sample.shape}"
                    assert y_sample.shape == (64, 64, 2), f"Bad Y shape: {y_sample.shape}"
