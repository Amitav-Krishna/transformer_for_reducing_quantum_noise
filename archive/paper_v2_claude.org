...
* Abstract
Can density-matrix denoisers benefit from architectures that model global correlations across quantum states? We compare convolutional and transformer autoencoders—matched at approximately 750,000 parameters—for denoising 5-qubit density matrices corrupted by five noise channels at four intensity levels. On average, noisy inputs retain only 0.11 Uhlmann fidelity to their clean targets. A transformer trained with a normalized Frobenius fidelity loss (analogous to cosine similarity for complex matrices) recovers 0.95 fidelity, while a CNN with the same loss reaches 0.30. Adding soft penalties for Hermiticity, positive semi-definiteness, and unit trace degrades both architectures: the physics-informed transformer drops to 0.33 fidelity, and the physics-informed CNN falls to 0.06—below the noisy baseline. These soft constraints appear to steer optimization toward generic states that satisfy physical axioms but lack target-specific structure. The transformer's advantage is consistent with—though does not prove—the hypothesis that global attention better captures the nonlocal correlations characteristic of entangled states. Our results also suggest that soft physics regularization may require careful tuning or alternative enforcement strategies to avoid degrading reconstruction quality.
* Introduction
Denoising quantum density matrices at the state level poses a fundamental question: which architectural inductive biases enable neural networks to recover the nonlocal correlations that define entangled quantum states? Existing approaches use convolutional networks or fully connected architectures, but these designs impose locality constraints or lack structure entirely—neither naturally captures the global coherences that distinguish quantum states from classical probability distributions. Meanwhile, prior work enforces physical validity (Hermiticity, unit trace, positive semidefiniteness) through hard architectural constraints, but whether /soft/ physics-informed regularization via loss penalties helps or hinders reconstruction remains untested.

This paper answers both questions through controlled experiments on synthetic density matrices corrupted by five noise channels. We find that transformers dramatically outperform CNNs: a transformer trained with normalized Frobenius fidelity loss achieves 0.95 mean Uhlmann fidelity on states whose noisy inputs have only 0.11 fidelity to the clean targets, while a CNN trained identically reaches only 0.30. More surprisingly, adding soft physics penalties degrades performance for both architectures—the physics-informed CNN falls to 0.06 fidelity, /worse than the noisy baseline/. These results establish that global self-attention is the critical architectural feature for density-matrix denoising, and that soft physics-informed regularization can be counterproductive.

Prior work motivates this comparison. Convolutional autoencoders have been applied to density-matrix denoising by treating quantum states as two-channel images, exploiting local spatial patterns in the real and imaginary components [cite:@kendre2025machinelearningquantumnoise]. Feedforward networks have demonstrated model-agnostic denoising of states corrupted by unknown dynamics [cite:@Morgillo_2024]. In the related domain of quantum error correction, transformer-based syndrome decoders outperform local CNN decoders precisely because self-attention provides global receptive fields that capture long-range correlations [cite:@wang2023transformerqecquantumerrorcorrection]. Reinforcement learning and optimal control approaches target robustness at the hardware level rather than state reconstruction [cite:@Ma_2025]. Unlike quantum error correction, which encodes logical information redundantly and corrects errors via syndrome measurements [cite:@Bravyi_2024; @202509.2149], or quantum error mitigation, which recovers accurate expectation values through circuit-level inference and post-processing [cite:@RevModPhys.95.045005; @Bultrini_2023; @filippov2024scalabilityquantumerrormitigation], our work targets direct reconstruction of corrupted density matrices without access to syndrome information or circuit structure.

We make three contributions: (1) the first controlled comparison of convolutional and transformer architectures for density-matrix denoising under identical training conditions; (2) evidence that global self-attention, not local convolution, provides the inductive bias suited to recovering quantum coherences; and (3) a demonstration that soft physics-informed regularization can actively harm reconstruction, suggesting that if physical validity is required, post-hoc projection may be preferable to training-time penalties.
*** Notes :noexport:
**** Kendre                                                        :noexport:
...
* Related Work
Recent work has explored neural network architectures for density-matrix denoising. Kendre et al. introduced a CNN-based approach that treats the density matrix as a two-channel image and applies encoder-decoder convolutions to map noisy states to clean reconstructions [cite:@kendre2025machinelearningquantumnoise]. This architecture exploits local spatial correlations, though its restricted receptive field may limit its ability to capture the global structure present in entangled states. Morgillo et al. applied feedforward networks to states corrupted by unknown noise channels, demonstrating that data-driven denoising can succeed without explicit noise models [cite:@Morgillo_2024]. Both approaches focused on demonstrating feasibility rather than comparing architectures with different inductive biases, leaving open the question of whether global-context models offer advantages for quantum state reconstruction.

A separate line of work addresses physical validity in quantum state tomography. Koutny et al. proposed positivity-constrained feedforward networks that project outputs onto valid quantum states, guaranteeing Hermiticity, unit trace, and positive semidefiniteness by construction [cite:@PhysRevA.106.012409]. This hard-constraint approach ensures physically meaningful outputs but has been evaluated primarily in tomographic settings where the network receives measurement data rather than corrupted states. Whether enforcing validity through soft regularization penalties—rather than architectural projection—helps or hinders denoising remains unexplored.

Transformer architectures have shown promise for quantum tasks involving global correlations. Wang et al. introduced Transformer-QEC for syndrome decoding in quantum error correction, where self-attention outperformed local CNN decoders by aggregating information across entire syndrome patterns [cite:@wang2023transformerqecquantumerrorcorrection]. Syndrome decoding and density-matrix denoising differ substantially: the former classifies discrete error patterns, while the latter reconstructs continuous matrix-valued states. Nevertheless, both tasks involve inputs whose structure spans the full system, suggesting that attention mechanisms warrant investigation for state reconstruction as well.

No prior work has directly compared convolutional and attention-based architectures for density-matrix denoising while controlling for model capacity and training procedure, nor systematically evaluated whether physics-informed loss penalties improve or degrade reconstruction quality. Our experiments address both questions.
* Methods
** Dataset 
...
\end{pmatrix}
$$
Phase damping models $T_2$ decoherence (loss of phase information without energy dissipation) and uses
...
This channel damps off-diagonal coherences by a factor $\sqrt{1-\gamma}$ while leaving populations unchanged, in contrast to the phase flip (dephasing) channel which applies $K_0 = \sqrt{1-p}I$, $K_1 = \sqrt{p}Z$.
Bit flip noise is
...
The mixed noise channel applies four channels sequentially in the following order: depolarizing, amplitude damping, phase damping, and bit-flip, each with strength /p/ /4. This ordering is fixed across all experiments. The mixed channel is not intended to correspond to a single effective noise process but to expose models to heterogeneous error mechanisms within a single circuit.
** Convolutional Autoencoder
The convolutional autoencoder follows the architecture introduced in [cite:@kendre2025machinelearningquantumnoise] for density-matrix denoising. The model treats each 32×32 complex-valued density matrix as a two-channel image (real and imaginary components). The encoder consists of three convolutional blocks, each comprising a 3×3 convolution with ReLU activation followed by 2×2 max-pooling, progressively reducing spatial resolution while expanding channel dimension (2→48→96→192). A bottleneck layer applies an additional 3×3 convolution at the compressed 4×4 resolution. The decoder mirrors the encoder through three blocks of nearest-neighbor 2× upsampling followed by 3×3 transposed convolutions with ReLU activations. A final transposed convolution projects the output back to two channels representing the real and imaginary components. The model has 748,898 trainable parameters.
** Transformer Autoencoder
The Transformer autoencoder models each 32×32 complex-valued density matrix as a sequence of 1024 token embeddings, where each token corresponds to one element in the row-vectorized matrix with real and imaginary parts concatenated into a 2-dimensional input vector. This architecture allows every matrix element to attend globally to every other element through self-attention.

We evaluate a capacity-matched Transformer with 751,186 parameters (comparable to the CNN's 748,898). A linear projection maps each 2-dimensional input to a 96-dimensional embedding. Learned absolute positional embeddings are added to distinguish matrix positions. The encoder consists of 4 Transformer encoder layers, each with 4 attention heads, feed-forward dimension 128, GELU activation, and 0.1 dropout. The encoded sequence passes through a symmetric bottleneck (linear 96→16→96) serving as a latent representation. The decoder comprises 4 Transformer decoder layers that apply self-attention within the output sequence and cross-attention to the encoder memory. A final linear projection maps the 96-dimensional embeddings back to 2 dimensions, and the sequence is reshaped to a 32×32×2 output.

We avoid patch embeddings to preserve element-level resolution; patching would impose a locality bias that conflicts with our architectural comparison.
** Normalized Frobenius Similarity Loss
The reconstruction loss is a normalized Frobenius inner product that measures alignment between predicted and target density matrices. For each predicted–target pair ($\hat{\rho}$, $\rho$), the loss is computed as
$$
...
$$
where $\langle \hat{\rho}, \rho \rangle_F = \mathrm{Tr}(\hat{\rho}^\dagger \rho)$ is the Frobenius inner product and $\lVert \cdot \rVert_F$ is the Frobenius norm. This formulation is analogous to cosine similarity for complex matrices: it measures the alignment between predicted and target states while normalizing for magnitude differences.

For valid density matrices (Hermitian, positive semi-definite), the Frobenius inner product $\mathrm{Tr}(\hat{\rho}^\dagger \rho)$ is always non-negative, so the normalized similarity lies in [0, 1] and the loss in [0, 1]. The loss achieves 0 when matrices are perfectly aligned and approaches 1 as alignment decreases. During training, we clamp the similarity to [-1, 1] to handle numerical edge cases from intermediate network outputs that may not satisfy density matrix constraints.

We emphasize that this metric is a /surrogate loss/ chosen for computational efficiency and gradient stability, not a measure of quantum state distinguishability. The Uhlmann fidelity $F(\rho, \sigma) = (\mathrm{Tr}[\sqrt{\sqrt{\rho}\sigma\sqrt{\rho}}])^2$ remains the operationally meaningful quantum fidelity, but its reliance on matrix square roots makes it prohibitively expensive to evaluate at each training step. While the relationship between normalized Frobenius similarity and Uhlmann fidelity is nonlinear (Fig. 1), we observe strong rank correlation between the two metrics (Spearman's $\rho$ = 0.95, p $\ll$ 10^{-10}), indicating that improvements under the Frobenius objective consistently correspond to improvements in quantum fidelity. All final evaluations report Uhlmann fidelity to ensure results reflect operationally meaningful reconstruction quality.
...
We note that the positive-semidefiniteness penalty requires eigenvalue computation, which introduces additional nonlinearity and may affect optimization stability; we return to this in the discussion.
* Experimental Design
We evaluate four denoising configurations arising from the factorial combination of two architectures (CNN, Transformer) and two loss functions (Frobenius similarity, physics-informed). This 2×2 design isolates the effects of architecture and loss function while controlling for model capacity.

All models are trained to map noisy density matrices to their clean counterparts. Training uses the Adam optimizer with learning rate 3×10^{-4}, weight decay 0, and a fixed budget of 100 epochs. Batch sizes differ due to memory constraints: 32 for the CNN and 8 for the Transformer. Checkpoints are selected based on minimum validation loss (10% of training data). All experiments use a fixed random seed (42) for dataset splitting and weight initialization to ensure reproducibility.

Evaluation is performed on the held-out test set (10% of data) using multiple metrics: Uhlmann fidelity between reconstructed and target states, normalized Frobenius similarity, trace deviation from unity, Hermiticity deviation ($\|\hat{\rho} - \hat{\rho}^\dagger\|_F$), and sum of negative eigenvalues ($\sum_{\lambda_i < 0} |\lambda_i|$). This combination assesses both denoising performance and physical validity.
** Hyperparameters
...
Model capacities are matched within 0.3% (748,898 vs 751,186 parameters). Different batch sizes accommodate memory constraints while keeping optimization settings otherwise identical.
** Compute resources
...
#+LATEX:\newpage
* Results
Transformers substantially outperform CNNs in reconstruction fidelity, while physics-informed losses degrade performance for both architectures. On the held-out test set, the Frobenius-trained transformer achieves 0.95 mean Uhlmann fidelity compared to 0.30 for the Frobenius-trained CNN—a threefold improvement relative to the 0.11 noisy baseline. Surprisingly, physics-informed regularization reduces fidelity for both architectures, with the physics-informed CNN falling below the noisy baseline entirely. These patterns are consistent across noise types and levels, as detailed in the subsections that follow.
*** Charts :noexport:
...
The fidelity comparison reveals a clear separation between architectural families. Models trained with normalized Frobenius similarity loss show the largest gap: the transformer achieves 0.95 fidelity compared to the CNN's 0.30, a factor of three improvement. One hypothesis for this gap is that the CNN's local receptive fields are poorly suited to modeling the global correlations present in entangled quantum states, whereas the transformer's self-attention mechanism can relate any matrix element to any other. However, we caution that this architectural explanation is suggestive rather than proven—the observed performance gap could also reflect differences in optimization dynamics, implicit regularization, or interactions between architecture and the specific noise channels tested.

Introducing the physics-informed loss degrades performance for both architectures. On the Frobenius proxy metric, the physics-informed CNN appears to benefit slightly (0.02 to 0.05), but as shown in the Uhlmann fidelity evaluation below, this is misleading: the true quantum fidelity actually decreases. The transformer loses substantial performance when physics constraints are applied (0.95 to 0.33 fidelity). The physics-informed loss penalizes deviations from Hermiticity, unit trace, and positive semidefiniteness. One possible explanation is that the soft constraints cause both architectures to converge toward outputs that satisfy physical axioms but have reduced alignment with the target state. However, this mechanism is hypothesized, not demonstrated—the degradation could also stem from optimization difficulties introduced by the eigenvalue-based PSD penalty or from the specific 0.1 weighting used.

These results show that (i) on this task, transformers substantially outperform CNNs for density-matrix denoising, and (ii) physics-informed regularization at the tested weight degrades reconstruction quality for both architectures when measured by Uhlmann fidelity.
...
* Conclusion
This work compared four machine learning strategies for density-matrix denoising on synthetic 5-qubit states corrupted by standard noise channels. The main findings are: (1) a capacity-matched transformer (751k parameters) substantially outperforms a CNN (749k parameters), achieving 0.95 mean Uhlmann fidelity versus 0.30; and (2) adding soft physics-informed regularization degrades reconstruction quality for both architectures, with the physics-informed CNN falling below the 0.11 noisy baseline.

These results were validated using Uhlmann fidelity, a standard quantum measure not directly optimized during training, reducing concerns about metric alignment. The transformer's advantage is consistent with the hypothesis that global self-attention better captures the correlations present in entangled states, though we cannot rule out alternative explanations related to optimization dynamics or architecture-noise interactions.

The physics-informed regularization finding is counterintuitive and warrants careful interpretation. The 0.1 weight and specific penalty formulation represent one point in a large design space—other weightings, alternative constraint formulations, or curriculum-based approaches might yield different results. We hypothesize that the soft penalties steer optimization toward generic states satisfying physical axioms rather than accurate reconstructions, but this mechanism remains unverified. If physical validity is required, post-hoc projection onto the valid state manifold may be worth exploring as an alternative to training-time regularization.

** Limitations
Several limitations constrain the scope of these conclusions. First, the dataset is entirely synthetic: density matrices were generated using Cirq simulations with idealized noise models that may not capture the complexity of real hardware noise. Second, the 5-qubit (32×32) scale is small relative to systems of practical interest, and it is unclear whether the transformer's advantage persists at larger scales where attention costs grow quadratically. Third, we tested only one physics regularization weight (0.1); ablation studies across weights could reveal regimes where physics constraints help rather than hurt. Finally, this study examines only state-level denoising given complete density matrices—practical quantum error mitigation typically operates on measurement data or circuit-level representations.

Despite these limitations, the substantial performance gap between transformer and CNN architectures suggests that global context mechanisms merit further investigation for quantum state reconstruction tasks. Extending this comparison to larger systems, realistic noise models, and measurement-based inputs would help clarify the practical applicability of these findings.
* References
#+print_bibliography:
