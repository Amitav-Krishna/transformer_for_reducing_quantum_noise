...
v18 key change: Seed ablation - retrain with different seeds for reproducibility.

Checkpoints saved to: train_18/checkpoints_18_seed{SEED}/
Training logs saved to: train_18/csvs_18_seed{SEED}/
...
# Local imports (self-contained in train_18/)
...
def train_single_experiment(name, model, arch, train_chunks, val_chunks, device, base_dir, seed):
    ...
    ckpt_dir = os.path.join(base_dir, f"checkpoints_18_seed{seed}", name)
    os.makedirs(ckpt_dir, exist_ok=True)

    # CSV logger with seed in name
    csv_dir = os.path.join(base_dir, f"csvs_18_seed{seed}")
    ...
    print(f"\n{'=' * 60}")
    print(f"Training {name} (v18 - seed ablation)")
    print(f"Seed: {seed}")
    ...
    print(f"\nTraining complete for {name} with seed {seed}")
    print(f"Best validation loss: {best_val:.6f}")
    return best_val


def main():
    parser = argparse.ArgumentParser(description="Train 5-qubit models with seed ablation")
    ...
    # Get base directory (train_18/)
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # Load and split dataset with the specified seed
    chunks = load_chunks("dataset_5qubit_float64")
    train_chunks, val_chunks, test_chunks = split_chunks(chunks, seed=args.seed)

    print(f"Train chunks: {len(train_chunks)}")
    print(f"Val chunks: {len(val_chunks)}")
    print(f"Test chunks: {len(test_chunks)}")
    print(f"\nUsing FROBENIUS NORMALIZATION for scale consistency")
    print(f"Using seed {args.seed} for train/val/test split")
    ...
    print(f"Checkpoints saved to: {os.path.join(base_dir, f'checkpoints_18_seed{args.seed}/')}")
    print(f"Training logs saved to: {os.path.join(base_dir, f'csvs_18_seed{args.seed}/')}")
    print("=" * 60)
