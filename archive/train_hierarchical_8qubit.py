"""
Train Hierarchical Transformer on 8-qubit dataset with STREAMING.

Uses StreamingChunkDataset to load chunks on-demand, avoiding OOM errors
for the 98GB 8-qubit dataset.

Key differences from 5-qubit training:
- Input: 256x256 matrices (vs 32x32)
- Batch size: 4 (vs 8) due to larger matrices
- Dataset: /workspace/dataset_8qubit/ (on runpod)
- STREAMING: Loads one chunk at a time

Expected runtime: ~12-15 hours for 100 epochs on RTX A4000.
"""
...
def main():
    ...
    if os.path.exists("/workspace/dataset_8qubit"):
        ...
        checkpoint_dir = "checkpoints_2/hierarchical_transformer_8qubit"
        csv_dir = "csvs_2"
    ...
    # Training config
    EPOCHS = 100
    BATCH_SIZE = 4  # Smaller batch due to 256x256 matrices
    ...
    # CSV loggers
    train_csv_path = os.path.join(csv_dir, "hierarchical_transformer_8qubit_train.csv")
    val_csv_path = os.path.join(csv_dir, "hierarchical_transformer_8qubit_val.csv")
    timing_csv_path = os.path.join(
        csv_dir, "hierarchical_transformer_8qubit_timing.csv"
    )
