"""
Hierarchical Transformer for 8-qubit density matrices (float64).

Reuses architecture from models/transformer_hierarchical_8qubit.py.
...
class PatchEmbed(nn.Module):
    """Convert 256x256 matrix to 64 patch tokens (32x32 patches)."""
...
class PatchUnembed(nn.Module):
    """Convert 64 patch tokens back to 256x256 matrix."""
...
class HierarchicalTransformer8Qubit(nn.Module):
    ...
    Parameters: ~1,611,072

    Key insight: Same token count (64) as 5-qubit model, so attention
    complexity is O(64^2) = O(4096) regardless of qubit count.
    """
...
if __name__ == "__main__":
    model = HierarchicalTransformer8Qubit()  # Already float64 by default
