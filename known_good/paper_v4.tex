...
\section{Methods}
\label{sec:orga80f457}

\subsection{Dataset Generation}
\label{sec:org73c5132}
...
\label{sec:org596d883}

\subsubsection{Shared Hyperparameters}
\label{sec:orgbf98fe1}
...
\section{Results}
\label{sec:org9812d11}

\subsection{Effect of Frobenius Normalization}
\label{sec:orgef7e911}

Our central finding is that Frobenius normalization dramatically improves training stability and final model performance. Table \ref{tab:orgf3e98cd} compares validation loss with and without normalization on the 5-qubit dataset.

\begin{table}[htbp]
\caption{\label{tab:orgf3e98cd}Effect of Frobenius normalization on 5-qubit MLP training. Normalization reduces validation loss by 25\% and enables stable convergence. *Unstable: capacity scaling hurts performance; see Appendix \ref{sec:org3875426}.}
\centering
\begin{tabular}{lrl}
...
\begin{table}[htbp]
\caption{\label{tab:orgc982503}Capacity ablations on unnormalized 5-qubit data. Larger models perform worse, indicating optimization pathology rather than underfitting.}
\centering
\begin{tabular}{llrl}
...
\label{sec:orgf95a84c}

With Frobenius normalization, both architectures achieve substantial improvements over the noisy baseline:

\begin{table}[htbp]
\caption{\label{tab:org2651d68}5-qubit model performance with Frobenius normalization. Both models use identical patch tokenization (\(4\times 4\) patches, 64 tokens) and matched parameter counts.}
\centering
\begin{tabular}{llrl}
Model & Params & Uhlmann Fidelity & vs Baseline\\
\hline
...
\begin{table}[htbp]
\caption{\label{tab:org18715d0}8-qubit scaling results with Frobenius normalization. Despite the exponentially harder task (baseline 0.006), both models achieve more than 3 times improvement.}
\centering
\begin{tabular}{lrl}
...
\label{sec:orgf6defc8}

\subsubsection{Patch Size Ceiling}
\label{sec:orga868da3}
...
\section{Appendix}
\label{sec:org9f78bf5}

\subsection{Model Parameter Counts}
\label{sec:orgde14113}
\begin{table}[htbp]
\caption{\label{tab:orgfec944a}Detailed parameter breakdown.}
\centering
\begin{tabular}{lllll}
...
\label{sec:org1b06136}
To validate Frobenius fidelity as a training proxy, we analyzed its correlation with the ground-truth Uhlmann fidelity. Figure \ref{fig:orgc026e35} shows the relationship for 100 randomly sampled 5-qubit density matrices.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/frob_vs_uhlmann_correlation.pdf}
\caption{\label{fig:orgc026e35}Correlation between Frobenius fidelity (normalized) and Uhlmann fidelity (physical) on a random subset of the 5-qubit dataset. The strong correlation (\(r=0.91\)) justifies using Frobenius fidelity as a training proxy.}
\end{figure}
\subsection{Single vs Double Precision}
\label{sec:org5d7df85}
...
\label{sec:org8a2eb01}

We scaled MLP hidden dimensions by 4 times, increasing parameters from 1.09M to 5.29M.

\begin{table}[htbp]
\caption{\label{tab:org9aee1b8}Capacity ablation without normalization: wider MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{llrl}
...
\begin{table}[htbp]
\caption{\label{tab:org513eabb}Depth ablation without normalization: deeper MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{lrlrl}
