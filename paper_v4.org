...
Both architectures achieve substantial improvements over the noisy baseline, with the MLP achieving $3.09\times$ improvement and the Transformer achieving $3.14\times$ improvement. Notably, with proper input normalization, the gap between MLP and Transformer narrows considerably---the Transformer's advantage is ~1.7% relative (0.525 vs 0.516). For the unnormalized experiments and the full comparison, see Appendix [[#app:pre-norm-ablations]].
