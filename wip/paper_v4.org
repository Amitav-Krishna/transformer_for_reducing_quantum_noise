...
* Abstract
Quantum computers offer asymptotic speedups for problems like molecular simulation and integer factorization, but noise severely limits their near-term utility.  Quantum Error Correction can address this, but requires too many ancillary qubits for current hardware.  Quantum Error Mitigation is a near-term alternative, but only recovers expectation values, not the full quantum state needed for tasks like process tomography or entanglement verification.  For these, we need quantum state reconstruction: recovering a clean density matrix from noisy tomographic measurements.  Neural networks have shown promise here, but previous approaches plateau at 5 qubits.  We find that Frobenius normalization (scaling inputs to unit purity) removes this bottleneck, enabling scaling to 8-qubit systems. 
