...
\addbibresource{/home/amitav-krishna/codage/transformer_for_reducing_quantum_noise/wip/references.bib}
\addbibresource{references.bib}
\author{Amitav Krishna\thanks{Correspondence: krishna@amitav.net}\thanks{This work has been submitted to IEEE for possible publication.  Copyright may be transferred without further notice, after which this version may no longer be accessible.}}
...
\caption{\label{fig:orga5886f9}Overview of the training and evaluation pipeline. Quantum states are measured, reconstructed via tomography, and normalized to unit Frobenius norm. A neural denoiser maps the normalized noisy state to a denoised estimate, which is then projected to a valid density matrix before computing Uhlmann fidelity.}
\end{figure*}
...
\begin{table}[htbp]
\caption{\label{tab:org449a1d3}5-qubit model performance with Frobenius normalization.  Both models use identical patch tokenization (\(4\times 4\) patches, 64 tokens) and matched parameter counts.}
\centering
\begin{tabular}{llrl}
...
\begin{table}[htbp]
\caption{\label{tab:orgb987053}Capacity ablations on unnormalized 5-qubit data.  Larger models perform worse, confirming an optimization pathology.}
\centering
\begin{tabular}{llrl}
Model & Params & Uhlmann Fidelity & vs Baseline\\[0pt]
\hline
...
\begin{table}[htbp]
\caption{\label{tab:org89343d7}8-qubit scaling results with Frobenius normalization.  Despite the exponentially harder task (baseline 0.006), both models achieve more than 3 times improvement.}
\centering
\begin{tabular}{lrl}
