...
\addbibresource{/home/amitav-krishna/codage/qnr/wip/references.bib}
...
\section{Methods}
\label{sec:org6df6761}

\subsection{Dataset Generation}
\label{sec:org78c2348}
...
\label{sec:org42e5acf}

\subsubsection{Shared Hyperparameters}
\label{sec:org9364179}
...
\section{Results}
\label{sec:org58c3a5b}

\subsection{Effect of Frobenius Normalization}
\label{sec:org54fd89a}

Our central finding is that Frobenius normalization dramatically improves training stability and final model performance. Table \ref{tab:orgb21798c} compares validation loss with and without normalization on the 5-qubit dataset.

\begin{table}[htbp]
\caption{\label{tab:orgb21798c}Effect of Frobenius normalization on 5-qubit MLP training. Normalization reduces validation loss by 25\% and enables stable convergence. *Unstable: capacity scaling hurts performance; see Appendix \ref{sec:orgcc7d2e5}.}
\centering
\begin{tabular}{lrl}
...
\begin{table}[htbp]
\caption{\label{tab:orgc90edb4}Capacity ablations on unnormalized 5-qubit data. Larger models perform worse, indicating optimization pathology rather than underfitting.}
\centering
\begin{tabular}{llrl}
...
\label{sec:org6e29263}

With Frobenius normalization, both architectures achieve substantial improvements over the noisy baseline:

\begin{table}[htbp]
\caption{\label{tab:org46edc70}5-qubit model performance with Frobenius normalization. Both models use identical patch tokenization (\(4\times 4\) patches, 64 tokens) and matched parameter counts.}
\centering
\begin{tabular}{llrl}
Model & Params & Uhlmann Fidelity & vs Baseline\\[0pt]
\hline
...
\begin{table}[htbp]
\caption{\label{tab:org851c7c1}8-qubit scaling results with Frobenius normalization. Despite the exponentially harder task (baseline 0.006), both models achieve more than 3 times improvement.}
\centering
\begin{tabular}{lrl}
...
\label{sec:org43b0d82}

\subsubsection{Patch Size Ceiling}
\label{sec:orgfc04d0b}
...
\section{Appendix}
\label{sec:org00a79c6}

\subsection{Model Parameter Counts}
\label{sec:orgb0da3a9}
\begin{table}[htbp]
\caption{\label{tab:orgbce332d}Detailed parameter breakdown.}
\centering
\begin{tabular}{lllll}
...
\label{sec:org4af30cf}
To validate Frobenius fidelity as a training proxy, we analyzed its correlation with the ground-truth Uhlmann fidelity. Figure \ref{fig:org15260a7} shows the relationship for 100 randomly sampled 5-qubit density matrices.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figures/frob_vs_uhlmann_correlation.pdf}
\caption{\label{fig:org15260a7}Correlation between Frobenius fidelity (normalized) and Uhlmann fidelity (physical) on a random subset of the 5-qubit dataset. The strong correlation (\(r=0.91\)) justifies using Frobenius fidelity as a training proxy.}
\end{figure}

\subsection{Single vs Double Precision}
\label{sec:orgf902578}
...
\label{sec:org9e9e903}

We scaled MLP hidden dimensions by 4 times, increasing parameters from 1.09M to 5.29M.

\begin{table}[htbp]
\caption{\label{tab:orge678df2}Capacity ablation without normalization: wider MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{llrl}
...
\label{sec:orgd8f102b}

We doubled the number of layers from 4+4 to 8+8.

\begin{table}[htbp]
\caption{\label{tab:orga01228b}Depth ablation without normalization: deeper MLP performs worse, confirming optimization pathology.}
\centering
\begin{tabular}{lrlrl}
