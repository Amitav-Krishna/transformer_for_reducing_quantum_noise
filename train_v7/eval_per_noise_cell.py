...
Run on runpod:
    python train_v7/eval_per_noise_cell.py
"""
...
def main():
    ...
    # Output directory
    out_dir = "train_v7/csvs_7/noise_cells"
    ...
    # === Residual MLP (v7) ===
    print("\nEvaluating Residual MLP (v7)...")
    mlp = MLPResidualAutoencoder(loss_fn=FrobeniusFidelityLoss()).to(device)
    mlp.load_state_dict(torch.load("checkpoints_7/mlp/best.pt", map_location=device))
    ...
    # === Transformer (v6) ===
    print("\nEvaluating Transformer (v6)...")
    transformer = TransformerAutoencoder(loss_fn=FrobeniusFidelityLoss()).to(device)
    transformer.load_state_dict(torch.load("checkpoints_6/transformer/best.pt", map_location=device))
