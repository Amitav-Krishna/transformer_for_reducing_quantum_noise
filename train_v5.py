"""
Train MLP and Transformer (v5) with unconstrained outputs.

v5 Transformer uses per-row output projection instead of global CLS projection.

Checkpoints saved to: checkpoints_5/
Training logs saved to: csvs_5/
...
def train_single_experiment(name, config, train_chunks, val_chunks, device):
    ...
    # Directories
    ckpt_dir = f"checkpoints_5/{name}"
    os.makedirs(ckpt_dir, exist_ok=True)

    # CSV logger
    csv_logger = CSVLogger(csv_dir="csvs_5", name=name)
    ...
    if arch == "mlp":
        ...
        BATCH = 16  # Per-row transformer can use larger batches
...
def main():
    ...
    os.makedirs("checkpoints_5", exist_ok=True)
    os.makedirs("csvs_5", exist_ok=True)
    ...
    print("Checkpoints saved to: checkpoints_5/")
    print("Training logs saved to: csvs_5/")
    print("="*60)
